...
int main(int argc,char **argv)
{
    ...
    cl::Buffer b_buffer(*clu_Context, CL_MEM_WRITE_ONLY, s * sizeof(int));
    ...
    clu_Queue->enqueueWriteBuffer(b_buffer, true, 0, s * sizeof(int), b);
    delete[] a;
    ...
    cl_int clerr = clu_Queue->enqueueNDRangeKernel(*kernel, cl::NullRange,
        ...
        cl::NDRange(size/s));   // group work size
    cluCheckError(clerr, "Error running the kernel");

    // copy back the results

    clu_Queue->enqueueReadBuffer(b_buffer, true, 0, s * sizeof(int), b);
